...
class Task0 extends JPanel implements ActionListener {
    ...
    public void init() {
        prompt = new Label ("Please Enter Multiplication Value:"); add(prompt);
        inputField = new TextField(15); add(inputField);
        submitButton = new Button("Submit"); add(submitButton);
    ...
    public void paint (Graphics g) {
        ...
        ypos = start;
        for (int i = 10; i >= 0; i--) {
            if (i > 0) { g.drawString("" + i, 60, ypos); }
            else { g.drawString("Blast Off!", 60, ypos); }
